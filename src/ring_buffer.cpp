...
// Non-volatile ring methods
size_t
RingBufferBytesRemaining(RingBuffer *buffer)
{
    return(buffer->size_in_bytes - buffer->next_index_to_write + buffer->next_index_to_read);
}

size_t
WriteBytesToRingBuffer(RingBuffer *buffer, void *src, size_t byte_count)
{
    size_t data_start_index = buffer->next_index_to_write;
...
void
ReadBytesFromRingBuffer(RingBuffer *buffer, void *dst, size_t byte_count)
{
    for(int i=0; i<byte_count; ++i)
    {
        ((u8*)dst)[i] = ((u8*)buffer->data)[(buffer->next_index_to_read + i) % buffer->size_in_bytes];
