    ...
    while(GetLastError() != ERROR_NO_MORE_FILES)
    {
        ...
        while(is_parsing and BufferBytesRemaining(file) > 0)
        {
            ...
            else if(token.type == TokenType_::Identifier)
            {
                ...
                if(TokenMatchesString(token, "NoImplementation"))
                {
                ...
                { // Try to parse from here as a struct.
                    ...
                    if(struct_is_templated)
                    {
                        ...
                        if(!no_implementation) fprintf(&out_file, "template<typename Type>\n");
                    ...
                    // struct members
                    while(!no_implementation and BufferBytesRemaining(file) > 0)
                    {
